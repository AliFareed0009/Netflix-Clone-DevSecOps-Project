...
pipeline{
    ...
    stages {
            ...
            steps{
                script{
                   withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){   
                       ...
                       sh "docker tag netflix alifareed0009/netflix:latest "
                       sh "docker push alifareed0009/netflix:latest "
                    }
                }
            }
        }
        stage("TRIVY"){
            steps{
                sh "trivy image alifareed0009/netflix:latest > trivyimage.txt" 
            }
        }
        stage('Deploy to container'){
            steps{
                sh 'docker run -d --name netflix -p 8081:80 alifareed0009/netflix:latest'
            }
        }
    }
}
...
**If you get a Docker login failed error and the pipeline fails, then use the below commands in EC2**
...
[DevSecOps Pipeline Project: Deploy Netflix Clone](https://youtu.be/g8X5AoqCJHc)
